...
% Calculate new climate parameters with applied changes 
% Ta_mod=1.0;
% P_mod=-0.18;
% RH_mod=-0.05;
...
% Loads Hypsometry data from castorBathymetry function
% constants for exponential outseepage function

load '../Data/CastorNewBathymetry.mat';
...
yeari=climate(mod(floor(t),sizedat)+1,3)+size(years,1)*loop_counter;
years(i)=yeari;
...
if yeari>=final_20thCentYear+floor(climate(end,3)-final_20thCentYear)/2
    if dayinyear >90 && dayinyear<274
        Tai=Tai+T_mod_summer;
    else
        Tai=Tai+T_mod_winter;
    end
    RHi=RHi+RHi*RH_mod;
    if RHi > 100
        RHi = 100;
    end
    if Pi > 0 && yearinloop > climate(1,3) && yearinloop < climate(end,3)
        if dayinyear > 90 && dayinyear < 274
            Pi=Pi+Pi*yearly_P_seasonality(yearly_P_seasonality(:,1)==yearinloop,5);
        else 
            Pi=Pi+Pi*yearly_P_seasonality(yearly_P_seasonality(:,1)==yearinloop,6);
        end
    end
end
...
if dayinyear>=summer_begin && dayinyear <summer_end
    ...
    if isnan(d18O_VPDB_aragonite) || isnan(d_L)
        break
    end
